      ...
      swap_chart(new QChart())
{
    ...
    QObject::connect(timer,SIGNAL(timeout()),this,SLOT(timerRefresh()));
    timer->start(1000);
...
void MainWindow::showInfo(int index){
    ...
    if(index==2){
        tempFile.setFileName("/proc/sys/kernel/hostname"); //打开hostname文件
        if ( !tempFile.open(QIODevice::ReadOnly) )
        {
        ...
        tempFile.setFileName("/proc/sys/kernel/ostype"); //打开ostype文件
        if ( !tempFile.open(QIODevice::ReadOnly) )
        {
            QMessageBox::warning(this, tr("warning"), tr("The hostname file can not open!"), QMessageBox::Yes);
            return;
        ...
        tempFile.setFileName("/proc/sys/kernel/osrelease"); //打开kernel版本信息文件
        if ( !tempFile.open(QIODevice::ReadOnly) )
        {
            QMessageBox::warning(this, tr("warning"), tr("The hostname file can not open!"), QMessageBox::Yes);
            return;
        ...
        tempStr = tempFile.readLine();  //start time to now; free time
...
void MainWindow::on_pushButton_4_clicked()
{
    ...
    //    system("sudo ~/Qt/Tools/QtCreator/bin/qtcreator");
    if(ok==-1){
