...
#define INODENUM 1024           // inode nums
#define BLOCKNUM (1024 * 64)    // disk size 64 MB
#define FILEBLKMAX (1024 * 4)   // file max size is 4 MB
...
#define USERNUMMAX 10          // user num max

#define INODESIZE sizeof(inode)                         // inode size 
#define SUPERPOS sizeof(sys_users)                      // #1 is super block 
#define INODEPOS (SUPERPOS + sizeof(super_block))       // #2 ~ #1025 are inodes 
#define BLOCKPOS (INODEPOS + INODESIZE * INODENUM)      // #1026 is first free block 
...
#define BUFFERFILE "/tmp/my_disk_buf"
